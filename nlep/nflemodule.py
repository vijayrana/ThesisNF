...
"""
TODO: file structure for all methods and editing sys args with function args
"""
...
def bitrateQuality(d, r):
	"""
		d: path to distroted video directory
...
if __name__ == '__main__':
	#generateSplitsFfmpeg(sys.argv[1])
	f = sys.argv[1]
	fdir = f.split('.')[1]
	resdir = createResoltuions(sys.argv[1])
	os.chdir(resdir)
	for f in os.path.join(os.getcwd(), resdir):
		if os.path.isfile(f) && f.endswith('.mkv'):
			bitrateQuality(f, sys.argv[1])
